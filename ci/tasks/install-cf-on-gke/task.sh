...
if [[ "${UPTIMER}" == "true" ]]; then
  ...
  set +e
  uptimer -useBuildpackDetection=true -configFile=/tmp/uptimer-config.json -resultFile=uptimer-result/${UPTIMER_RESULT_FILE}
  uptimer_exit_code=$?
  set -e

  if [[ "${EMIT_UPTIMER_METRICS_TO_WAVEFRONT}" == "true" ]]; then
    source runtime-ci/tasks/shared-functions
    push_uptimer_metrics_to_wavefront "${SOURCE_PIPELINE}" "${UPTIMER_RESULT_FILE}"
  fi
