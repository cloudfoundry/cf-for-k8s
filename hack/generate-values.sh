...
- name: instance_index_env_injector_certificate
  ...
  options:
    ...
    extended_key_usage:
    - server_auth
- name: resource_validator_certificate
  type: certificate
  options:
    ca: default_ca
    common_name: "*.cf-system.svc"
...
resource_validator_certificate:
  #! This certificates and keys should be valid for *.cf-system.svc
  crt: |
$(bosh interpolate ${VARS_FILE} --path=/resource_validator_certificate/certificate | grep -Ev '^$' | sed -e 's/^/    /')
  key: |
$(bosh interpolate ${VARS_FILE} --path=/resource_validator_certificate/private_key | grep -Ev '^$' | sed -e 's/^/    /')
  ca: |
$(bosh interpolate ${VARS_FILE} --path=/resource_validator_certificate/ca | grep -Ev '^$' | sed -e 's/^/    /')
