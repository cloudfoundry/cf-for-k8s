...
var _ = Describe("Missing Attributes", func() {
	...
	Context("when all required attributes are missing", func() {
		...
		It("should list all the required attributes", func() {
			Expect(ctx).To(ThrowError(`The following required data.values parameters are missing: \["app_domains", "app_registry.hostname", "app_registry.password", "app_registry.repository_prefix", "app_registry.username", "blobstore.secret_access_key", "capi.cc_username_lookup_client_secret", "capi.cf_api_controllers_client_secret", "capi.database.adapter", "capi.database.encryption_key", "capi.database.host", "capi.database.name", "capi.database.password", "capi.database.port", "capi.database.user", "cf_admin_password", "cf_db.admin_password", "gateway.https_only", "internal_certificate.ca", "internal_certificate.crt", "internal_certificate.key", "system_certificate.ca", "system_certificate.crt", "system_certificate.key", "system_domain", "uaa.admin_client_secret", "uaa.database.adapter", "uaa.database.host", "uaa.database.name", "uaa.database.password", "uaa.database.port", "uaa.database.user", "uaa.encryption_key.passphrase", "uaa.jwt_policy.signing_key", "uaa.login_secret", "uaa.login.service_provider.certificate", "uaa.login.service_provider.key", "workloads_certificate.ca", "workloads_certificate.crt", "workloads_certificate.key"\]`))
		})
	...
	Context("when some required attributes are missing", func() {
		...
		It("should complain about missing attributes", func() {
			Expect(ctx).To(ThrowError(`The following required data.values parameters are missing: \["app_registry.username", "capi.cc_username_lookup_client_secret", "capi.cf_api_controllers_client_secret", "cf_admin_password", "cf_db.admin_password", "gateway.https_only", "internal_certificate.ca", "internal_certificate.key", "system_certificate.key", "uaa.database.adapter", "uaa.database.host", "uaa.database.name", "uaa.database.port", "uaa.database.user", "uaa.encryption_key.passphrase", "uaa.jwt_policy.signing_key", "uaa.login_secret", "uaa.login.service_provider.certificate", "uaa.login.service_provider.key", "workloads_certificate.ca"\]`))
		})
	})
